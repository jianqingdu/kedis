start_server {tags {"dump"}} {
    ...
    test {RESTORE set an arbitrary expire to the materialized key has no effect} {
        r set foo bar
        set encoded [r dump foo]
        r del foo
    ...
    test {RESTORE can overwrite an existing key} {
        r set foo bar1
    ...
    test {MIGRATE is able to migrate a key between two instances for list} {
        set first [srv 0 client]
        r del list
        ...
        start_server {tags {"repl"}} {
            ...
            assert_equal {d c b a} [$second lrange list 0 -1]
        }
    }
	...
	test {MIGRATE can correctly transfer zset} {
        set first [srv 0 client]
        r del key
        r zadd key 1 a 2 b 3 c 4 d 5 e 6 f
        start_server {tags {"repl"}} {
            set second [srv 0 client]
            set second_host [srv 0 host]
            set second_port [srv 0 port]
            ...
            assert {[$second exists key] == 1}
            assert {[$second ttl key] == -1}
			assert_equal {a b c d e f} [$second zrange key 0 -1]
        }
    }
